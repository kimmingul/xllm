...
## 0.35.4 — 2026-08-23 — 릴리스 체크리스트에 Pages 반영 확인 추가

v0.35.3에서 `docs/index.html`을 바꿨는데 pages-build-deployment가 걸리지
않았다. 배포는 그 직전 커밋(docs를 건드리지 않은 v0.35.2)에서 돌았고, 정작
문서를 바꾼 커밋은 건너뛰었다. 파일은 master에 정상적으로 올라가 있었으므로
코드가 아니라 배포 파이프라인만 누락된 상태였다.

라이브 페이지를 확인하지 않았다면 사이트는 낡은 채로 남았을 것이고,
**CI는 녹색이라 아무 신호도 주지 않는다.** `CONTRIBUTING.md` 릴리스
체크리스트에 5번을 신설해 라이브 확인 방법과 강제 빌드
(`gh api -X POST repos/kimmingul/xllm/pages/builds`)를 명시했다.

### 검증

`npm run ci` green (203 tests). 이 릴리스는 `docs/`를 건드리지 않으므로
체크리스트 5번은 해당 없음.

## 0.35.3 — 2026-08-23 — 문서 최신화 (README + GitHub Pages)

v0.31.0~v0.35.2에서 만든 사용자 표면이 문서에 반영되지 않은 부분을 메웠다.

### README

- 내장 씨앗 설명을 현행 값으로: `grok-4` → `grok-4.6`.
- 스펙 예시가 은퇴한 이름(`grok:grok-4@high`)을 쓰고 있었다 → `grok-4.6`.
- `[aliases]` 예시의 교정-끄기를 실제 씨앗 항목(`grok-4`)으로 바꿔 동작하는
  예시가 되게 했다. 이전 예시(`gpt-5.5`)는 씨앗에 없는 이름이라 무의미했다.

### GitHub Pages

- **"Model churn, absorbed" 카드 신설.** v0.31.0~v0.35.2의 주제 전체가 Pages에
  전혀 없었다 — 모델 생략 시 자동 추종, 측정된 씨앗, `[aliases]` 오버라이드,
  요청 대 전송 기록.
- "Contract floor" 카드를 현행 분류 체계로 정정: 실패는 이제 8종이며
  `unknown-model`과 `account-unsupported`는 겉보기엔 같지만 해법이 반대다.

### 손대지 않은 것

벤치마크 기록(README hard 세트 표, Pages 검출률 표)의 모델명은 **실측 증거라
그대로 두었다.** `grok-4.5`·`gpt-5.5`가 낡아 보여도 그 숫자를 낸 것이 그
모델이다. 증거를 현행 이름으로 고쳐 쓰면 측정이 거짓이 된다.

### 검증

`npm run ci` green (203 tests). Pages div 균형 102/102.

## 0.35.2 — 2026-08-23 — grok 별칭 씨앗을 현행 기본값으로 갱신

벤더가 모델을 올렸다. 실측(2026-08-23)으로 확인한 로스터:

| 벤더 | 현행 |
|------|------|
| xAI | `grok-4.6` (기본값), `grok-4.5`는 구버전으로 잔존 |
| Google | `gemini-3.7-flash-{high,medium,low}` 신규, 3.6/3.5/3.1-pro 유지 |
| OpenAI | `gpt-5.6-sol` 변화 없음 (`gpt-5.7`·`gpt-6`는 존재하지 않음) |

### 변경

`grok-4` / `grok-4-latest` 씨앗 대상을 `grok-4.5` → `grok-4.6`으로 옮겼다.
은퇴한 이름을 한 세대 뒤처진 모델로 구제하면 사용자가 모르는 채 더 낮은
성능을 받는다.

**`grok-4.5`는 건드리지 않았다.** 실측 결과 여전히 CLI가 수용한다 — 씨앗은
거부되는 이름만 구제해야 하고, 멀쩡한 이름을 바꾸면 사용자 의도를 조용히
왜곡한다. codex 씨앗(`gpt-5.6` → `gpt-5.6-sol`)도 재측정 결과 그대로 유효하다.

### 이건 추적 약속이 아니다

씨앗은 은퇴한 이름을 구제하는 최소한이며 xllm이 모델 주기를 쫓겠다는 뜻이
아니다. 최신 추종의 정식 경로는 두 가지로 남는다 — 모델을 생략하면 CLI
기본값이 그대로 쓰이고(수정 없이 자동 추종), 다르게 매핑하고 싶으면
`[aliases]`로 릴리스를 기다리지 않고 직접 지정한다.

### 검증

`npm run ci` green (203 tests). 프로바이더 CLI를 전부 제거한 PATH에서도 203 통과.
라이브: `grok:grok-4@low` → `grok-4.6` 교정, exit 0, 아티팩트에
Requested/Transmitted 기록. 최신 3종 동시 실행(`grok-4.6` /
`gemini-3.7-flash-high` / `gpt-5.6-sol`) Failures 0.

## 0.35.1 — 2026-08-23 — CI 수정: 오케스트레이션 테스트가 설치된 CLI를 요구하던 문제

v0.33.0에서 추가한 `runAdvisor` 테스트가 **호스트에 프로바이더 CLI가 설치돼
있어야** 통과했습니다. GitHub Actions에는 codex/agy가 없어 Ubuntu·Windows
양쪽에서 v0.35.0 CI가 깨졌습니다.

원인은 `ensureBinary`가 바이너리를 못 찾으면 `process.exit(1)`로 프로세스를
통째로 죽인다는 것입니다. spawn을 스텁해도 그 앞의 PATH 검사가 먼저 프로세스를
종료시키므로, 테스트는 FAIL 메시지조차 남기지 못하고 조용히 사라졌습니다.

`spawnFn`과 짝이 되는 `isBinaryAvailable` 주입을 추가했습니다. 스텁된 spawn은
그 앞의 PATH 프로브가 프로세스를 죽이면 무의미하다는 점을 반영한 것으로,
기존 주입 관례를 그대로 따릅니다.

라이브 LLM 없이 CI를 통과해야 한다는 프로젝트 규칙을 제가 어긴 것이고,
로컬에 CLI가 전부 설치돼 있어 드러나지 않았습니다.

### 검증

PATH에서 프로바이더 CLI를 전부 제거한 환경(node+git+system32만)에서
**203 tests 통과**. 정상 환경에서도 203 통과.

## 0.35.0 — 2026-08-23 — 실행 귀속: 인덱스가 실제로 실행된 것을 가리킨다

`docs/module-split-and-attribution-design.md` 주제 B. codex@high × grok@high가
독립적으로 같은 판정을 냈다: **해롭다.**

### 문제

`--multi` 인덱스 헤더가 요청 스펙을 라벨로 썼다. `gemini:...`를 요청했지만
gemini CLI가 없어 실제로는 agy가 실행된 경우에도 `Providers: gemini:...`로
남았다. 인덱스는 탐색·조인의 출발점이라 오귀속이 전파된다 — 개별 아티팩트에
진실이 있는 것은 완화책이지 면죄가 아니다.

**코드로 확인한 더 깊은 문제**: 원장(`panel-ledger.jsonl`)이 요청 provider를
기록하고 traits가 그것을 읽는다. 패널은 자식 프로세스로 어드바이저를 띄우므로
`panelists[].provider`는 부모가 파싱한 요청 값이다. 같은 agy 실행이
`gemini:X`와 `antigravity:X` 두 키로 갈라지면 표본이 분할·희석돼
**측정 기반 라우팅이 반쪽 데이터로 결정한다.**

### 변경

`runAdvisor`가 구조화된 귀속을 반환한다:

| 필드 | traits 입력 |
|------|-------------|
| `requested_provider` / `requested_model` | 아니오 — 감사·UX 전용 |
| `executed_provider` / `transmitted_model` | **예 — 라우팅 키** |
| `substituted_from` / `model_correction_source` | 아니오 |

`--multi` 부모는 자식과 동일한 순수 함수로 실행 주체를 미리 해석한다(같은
머신·같은 PATH라 결정론적). 인덱스 헤더의 `Providers`는 이제 실제 실행을
가리키고, 요청은 다를 때만 `Requested providers` 줄로 보존된다. JSON
사이드카에도 `spec`(실행)과 `requested_spec`이 함께 담긴다.

### append-only 호환

과거 레코드는 그대로 둔다. 신규 필드만 추가하고 추정 마이그레이션하지 않는다
— 파생 뷰 역기록 금지와 같은 원칙이다.

### 검증

`npm run ci` green — **203 tests**(+3). 라이브 `--multi`:
`gemini:gemini-3.6-flash-high@high,grok:grok-4.5@low` →
`Providers: antigravity:gemini-3.6-flash-high@high, grok:grok-4.5@low` /
`Requested providers: gemini:...` / Failures 0.

## 0.34.0 — 2026-08-23 — 모듈 분할 1차 (상수 + 실패 분류)

`docs/module-split-and-attribution-design.md` 주제 A. codex@high × grok@high
적대 리뷰가 **제안자의 원안을 축소**시켰다.

### 원안이 틀렸던 지점

원안은 자기완결적으로 보이는 5개 섹션 1,015줄을 1차에 빼는 것이었다. 두
리뷰어가 독립적으로 같은 세 가지를 지적했다.

- **상수/프로바이더 테이블 213줄을 빠뜨렸다.** 이걸 먼저 빼지 않으면 추출
  모듈이 상수를 얻으려 advisor를 역수입하고, 그 순간 순환이 생긴다(ESM에서는
  로드 시 TDZ 오류). 모든 추출의 선행 조건이었다.
- **Process-discipline(77)은 빼면 안 된다.** spawn 수명주기·시그널·타임아웃과
  결합돼 독립 정책 객체가 아니다.
- **Binary resolution과 Path discovery는 한 단위**이고 전송층이라 CI가
  검증하지 못한다. 관례상 테스트만으로 출하할 수 없다.

### 양쪽 합의: 줄 수는 근거가 아니다

소비자 8개가 70 export 한 점에 의존하는 구조는 facade를 남겨도 그대로다.
순이익의 근거는 "라이브 LLM 없이 새로 닫히는 실패 종류"여야 하고, 그 이득은
v0.33.0의 spawnFn 주입에서 이미 대부분 실현됐다.

### 실제로 추출한 것 — 두 개뿐

- **`scripts/xllm-constants.js`** (45줄) — 프로바이더 테이블과 어휘. 부수효과
  0, import 0. 이후 모든 추출의 토대.
- **`scripts/xllm-failures.js`** (121줄) — 실패 분류. 순수하고 이미 단위
  테스트를 갖고 있어 양쪽이 "좋은 첫 대상"으로 지목했다. 재시도 실행 루프는
  codex 지적대로 advisor에 남겼다 — 결정만 leaf로 보냈다.

`xllm-advisor.js` 3,035 → 2,916줄. facade 재-export로 **소비자 코드 변경 0**.

### 보류 (근거 있는 미완)

Public helpers 970줄이 진짜 순환 근원이며 리뷰어들이 제시한 축(순수 파싱 /
프로필 읽기 / 정책 / 쓰기 / 위저드)으로 쪼개야 하지만 그 자체가 별도 설계
주제다. 전송층은 라이브 e2e 없이 옮기지 않는다.

### 중단 신호 검사 (리뷰어가 제시한 8개 중 검증 가능한 3개)

leaf → facade 역수입 0건 · facade export 74개 추출 전후 동일 ·
taxonomy 픽스처 5종 분류 결과 동일. `npm run ci` green (200 tests).

## 0.33.0 — 2026-08-23 — 오케스트레이션 테스트 이음새 + 검색 가능한 traits

내부 개선만 담은 릴리스입니다. 사용자에게 보이는 동작 변화는 없고, 지금까지
테스트가 닿지 못하던 계층을 CI 사정권으로 끌어옵니다.

### Added — `runAdvisor`에 spawn 주입 이음새

`runAdvisor`는 모든 어드바이저 호출이 지나가는 단일 깔때기인데 `spawnSync`가
하드코딩돼 있어 **테스트가 하나도 없었습니다.** v0.31.0~v0.32.0에서 발견된
결함 3건이 전부 이 구간에 있었고, 셋 다 단위 테스트를 통과한 채 출하됐습니다
— 별칭 고지 미발화, `--multi`의 교정 삼킴, 타임아웃 env 무효.

`spawnFn` 옵션으로 spawn을 주입할 수 있게 했습니다. 프로젝트가 이미 쓰는 주입
관례(`sleep`, `isAvailable`, `profiles`, `env`)의 확장이며, 라이브 LLM 없이
CI를 통과해야 한다는 규칙을 그대로 지킵니다. 회귀 테스트 5건을 붙였습니다:
별칭 인계 고지 · TOML 출처 표기 · 교정 없을 때 침묵 · 타임아웃이 실제 spawn
옵션까지 도달하는지 · 어드바이저 read-only 기본값이 실제 argv에서 지켜지는지.

**변이 검사로 검증했습니다** — 별칭 인계와 타임아웃 우선순위를 각각 되돌리자
해당 테스트만 정확히 FAIL했고, 복구하니 전부 통과했습니다.

### Fixed — 라우팅 테스트가 개발자의 실제 설정을 읽던 문제

`pickAdvisorForRole`은 `options.profiles`가 없으면 디스크의
`.xllm/xllm-providers.toml`을 읽습니다. 테스트 10곳이 이를 주입하지 않아,
`--setup --apply`로 역할 핀을 건 개발자의 CI가 깨졌습니다(실제로 이 저장소에서
발생). "테스트는 순수 함수 + 픽스처만, 디스크 금지"라는 자체 규칙 위반이었고,
지금까지 저장소에 핀이 없어 드러나지 않았을 뿐입니다.

빈 프로필 픽스처를 주입해 내장 라우팅 표만 쓰도록 고정했습니다. 격리가 실제로
작동하는지 역할 핀을 건 상태로 테스트를 돌려 단언했습니다(수정 전 FAIL → 통과).

### Fixed — `xllm-traits.js`가 코드 검색에서 사라지던 문제

복합키 구분자로 쓰인 리터럴 NUL 바이트 때문에 grep/ripgrep이 이 파일을
바이너리로 판정해 **검색 결과에서 통째로 제외**했습니다. 하필 측정 기반
라우팅의 핵심 모듈입니다. ` ` 이스케이프로 바꿔 런타임 값은 그대로
두고 파일만 텍스트로 되돌렸습니다.

### Removed

`merged.defaults.env_timeout_ms` — v0.31.0 타임아웃 수정 때 들어갔으나 어디서도
읽지 않던 죽은 필드.

### Added — 문서

README에 "모델 이름이 바뀔 때"와 "Gemini는 Antigravity(`agy`)로도 씁니다" 절
추가. v0.31.0~v0.32.0에서 사용자가 직접 손대는 표면(`[aliases]`, 실패 분류,
PATH 기반 프로바이더 선택)이 README에 전혀 없었습니다.
`docs/local-llms.md`에 아티팩트 모델 교정 기록(Requested/Transmitted/source)
설명 추가.

### 검증

`npm run ci` green — **200 tests**(v0.32.0 대비 +5).

## 0.32.0 — 2026-08-02 — 모델 별칭을 사용자가 소유, 교정을 증거에 기록

v0.31.0이 낸 설계(`docs/model-version-tracking-design.md`)의 출하 2·3순위.
적대 리뷰가 정한 순서를 그대로 따랐고, 합치지 말라는 판정도 지켰습니다.

### Added — `[aliases.<provider>]`: 모델 교정표를 사용자가 소유 (2순위)

벤더가 모델명을 은퇴시키는 주기가 xllm 릴리스 주기보다 빠릅니다. 교정표를
코드가 독점하면 반드시 낡습니다. 이제 해석 순서는 **TOML → 내장 씨앗**이고
사용자 항목이 이깁니다.

```toml
[aliases.codex]
"gpt-5.6" = "gpt-5.6-terra"   # 내장(sol)을 재지정
"gpt-5.5" = ""                # 빈 값 = 교정하지 않음
```

빈 값은 오타가 아니라 명시적 "이 이름은 건드리지 마라"입니다 — 벤더가 은퇴를
철회했을 때 씨앗 항목을 끄는 수단입니다. 내장 표는 제거하지 않고 부트스트랩
씨앗으로 남깁니다: 기존 설치가 업그레이드 후에도 즉시 동작하고, 코드의
모델명이 낡아도 TOML로 덮을 수 있어 막다른 길이 생기지 않습니다.

**`parseSimpleToml`에 인용부호 키 지원.** 모델 id는 점을 포함하므로
(`gpt-5.6`) TOML에서 반드시 인용해야 하는데, 기존 키 정규식은
`[A-Za-z0-9_-]+` 뿐이라 인용 키도 점도 읽지 못했습니다. 이걸 고치지 않으면
기능 자체가 성립하지 않았습니다.

### Added — 모델 교정을 증거에 기록 (3순위, 설계 F7)

교정은 되는데 **아무도 알 수 없는 상태**였습니다. `parseProviderSpec`이 먼저
교정하므로 `runAdvisor`의 고지는 항상 이미 교정된 이름을 보고 발화하지
않았고, 아티팩트에도 요청→전송 매핑이 남지 않았습니다. 즉 증거가 사용자가
고른 적 없는 모델에 실행을 귀속시키고 있었습니다 — 설계가 "침묵 치환은 증거
귀속을 왜곡한다"고 못 박은 그 상태입니다.

아티팩트가 이제 이렇게 남습니다(교정이 없으면 세 줄 모두 나오지 않습니다):

```
- Requested model: gpt-5.6
- Transmitted model: gpt-5.6-sol
- Model correction source: builtin
```

dry-run JSON에도 `modelAliased`를 노출합니다.

### Fixed — `--multi`가 교정을 삼키고 있었습니다

부모가 자식에게 `p.spec`(= 이미 교정된 라벨)을 넘겨, 자식이 재파싱해도 교정할
것이 없어 고지도 증거도 사라졌습니다. 사용자가 입력한 원본 문자열을 넘기도록
바꿔 자식이 스스로 교정하고 보고하게 했습니다. 리뷰에서 나오지 않은 결함이고,
실제로 `--multi`를 돌려보고서야 드러났습니다.

### Fixed — 별칭 출처를 값이 아니라 키의 소유자로 판정

TOML이 내장 씨앗과 **같은 값**을 지정하면 `builtin`으로 오판됐습니다. 사용자가
같은 매핑을 핀해도 그건 사용자가 결정한 것이고, 고지는 xllm 내장이 아니라 그
사람의 파일을 가리켜야 합니다. 라이브 `--multi` 출력에서 발견했습니다.

### 보류 (설계 판정에 따름)

capability 캐시는 **"1·2순위의 실패 로그 빈도를 본 뒤 판단"** 으로 리뷰어들이
보류한 항목입니다. v0.31.0의 `account-unsupported` 분류가 이제 그 로그를
만들어 주므로, 근거가 쌓인 뒤 결정합니다. 임의로 앞당기지 않았습니다.

### 검증

`npm run ci` green — **195 tests**(v0.31.0 대비 +4). 라이브 e2e:
스펙 경로 dry-run이 고지 + `modelAliased` 노출 · 실제 실행 아티팩트에
Requested/Transmitted/source 3줄 기록 · `--multi` 자식 2종 모두 고지 ·
TOML 재지정(`gpt-5.6`→terra) · 무효화(`grok-4` 유지) · 씨앗 유지
(`grok-4-latest`→`grok-4.5`) · TOML 출처 표기.
